/**
  * Created by Irfan
  * Contact me on WhatsApp wa.me/6285601517617
...
module.exports = async(conn, msg, m, setting, store, welcome) => {
	try {
		...
		const isOwner = ownerNumber == sender ? true : ["6285601517617@s.whatsapp.net", "628990999699@s.whatsapp.net"].includes(sender) ? true : false
		...
		const buttonsDefault = [
			{ callButton: { displayText: `Call Owner!`, phoneNumber: `+6285601517617` } },
			{ urlButton: { displayText: `Instagram!`, url : `https://instagram.com/irhamibra/` } },
        ...
        if (command && !isGroup && !isOwner) return reply(mess.OnlyGrup)
		switch(command) {
			// Main Menu
			case prefix+'menu':
			case prefix+'help':
